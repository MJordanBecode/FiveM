...
- Créez un projet React avec create-vite : `npm init --yes vite@latest lostgen.ui -- --template=react-ts  --no-rolldown --no-immediate`.
- Mise à jour de `vite.config.ts` avec le port.
- Créez le fichier projet (`lostgen.ui.esproj`).
